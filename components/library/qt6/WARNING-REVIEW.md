...
head `e54c6f0ce96856ff135f5a3a3a05e4f860e9547b`, on 2026-09-12, with PR status
...
Configuration reproduced the policy, bundled CMake and skipped-module warnings
described below. Native compilation, prototype staging and private IPS publication
all completed with exit 0 on 2026-09-13. Compilation reproduced all 68
compiler/linker warnings listed below; native packaging lint also reproduced the
398 + 48 warnings.

Published package:
`pkg://userland/library/qt6@6.11.2,5.11-2026.0.0.0:20260913T050406Z`.
The exported `qt6-6.11.2-amd64.p5p` archive is 184,729,600 bytes, with SHA256
`21a947ccd8983aaf77671a62cff0497e55ae666a7068cbac801660f3b0a6a96d`.
No tracked recipe or manifest changes were generated by the build.

Staged qmake, rcc, moc and QDoc report Qt `6.11.2`. QDoc passes `ldd -r`, resolving
Qt from the new prototype and Clang/LLVM from `/usr/clang/21/lib`. The build-tree
XCB platform library also passed `ldd -r`. The recipe disables its test target;
these checks do not establish installed desktop, consumer or SPARC runtime results.
...
4. Complete consumer and desktop smoke tests for the coordinated release, and
   obtain SPARC evidence before making SPARC runtime claims. The native result
   here establishes an amd64 package build and the specific tool/loader checks.

## Compiler and linker warnings

The fresh native build and Jenkins builds 2 and 22 each emitted the same 68 Qt6
...
Native packaging lint and Jenkins builds 2 and 22 each emitted these warnings:
...
## Staging recipe cleanup

Both native staging and Jenkins build 22 also print three messages that are not
included in the compiler/lint warning counts:

- Remove the duplicate `qdbusviewer` item from the symlink-creation loop. Its
  second invocation prints `File exists`; the first creates a valid
  `qdbusviewer-qt6` link to the staged tool.
- Remove the stale `PYTHON_SCRIPTS` entries for `sanitizer-testrunner.py` and
  `qt-testrunner.py`. They exist in the source but are not installed by this
  recipe or listed in its manifest. Python's compilation step prints `Can't list`
  for both absent destinations. This cleanup makes the recipe match its actual
  package contents.
